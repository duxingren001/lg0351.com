...
人人商城复制商品给多商户用户user
调用 
daoru()方法：传值 商户id ，即可把总店的商品复制给多商户的任意商户。
...

{
...

	{
		...
		$shanghuid = $_GPC['merchid'];
		
		
		
		$goods = pdo_fetchall('SELECT  * FROM    ims_ewei_shop_goods  where  status = 1 and  merchid = 0  order by id desc');
...
/* 		
		...
		foreach($goods  as  $key=>$v){
			...
			for($i=0;$i<count($vkey);$i++){
				$data[$vkey[$i]] = $v[$vkey[$i]];
			}

$data['id'] = null;
$data['merchid'] = $shanghuid;
pdo_insert('ims_ewei_shop_goods', $data);
